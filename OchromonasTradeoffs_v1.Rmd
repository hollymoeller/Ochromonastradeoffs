---
title: "Evidence for tradeoffs between photosynthetic and phagotrophic investments in the mixotrophic chrysophyte Ochromonas"
...
PAdata$P_I.perC <- PAdata$chl.to.C*PAdata$P_I*Cfix.mult*12 # multiply by 12 because 12hrs light in each day 
PAdata$P_I.percell <- PAdata$chl.percell*PAdata$P_I*Cfix.mult*12 # multiply by 12 because 12hrs light in each day
...
PAdata$attack.per.C.scaled <- PAdata$attack.per.C*24*1000*1000 # scaled to per g carbon per day
...
# Explaining growth rates with photosynthesis and heterotrophy


```{r, fig.height=2.5,fig.width=2.5,warning=F}

par(mar=c(4,4,1,1),mfrow=c(1,1))

# PHOTOSYNTHESIS PREDICTS GROWTH: PHOTOSYNTHESIS PER CARBON

plot(PAdata$Growth.Rate~PAdata$P_I.perC,col=col.strains[as.factor(PAdata$Strain)],las=1,xlab='Photosynthetic Rate (pg C per pg C per d)', ylab='Growth Rate (per day)',lwd=.5,pch=21,type='n')
...
plot(PAdata$Growth.Rate~PAdata$C.from.grazing.perCperday,col=col.strains[as.factor(PAdata$Strain)],las=1,xlab='Phagotrophy Rate (pg C per pg C per d)', ylab='Growth Rate (per day)',lwd=.5,pch=21,type='n',log='x',xlim=c(0.01,50),ylim=c(0.15,1))
...
rainbowplot(PAdata[PAdata$Light>0,],'attack.per.C.scaled','Attack rate (per g C per day)','Light Level (grouped by strain)',spacer,strains,NA,'topleft','y',c(.08,20),toplabomit='T',xlab.cex = .7)
...
rainbowplot(PAdata[PAdata$Light>-10,],'Fv.Fm','Photosynthetic Efficiency (Fv/Fm)','',spacer,strains,NA,'bottomleft','n',xlab.cex=.7,xtickomit = T)
mtext('(a)',side=1,at=-205,line=-17.5)
...
mtext('(b)',side=1,at=-205,line=-17.5)
...
mtext('(c)',side=1,at=-205,line=-17.5)
...
legend(x='topright',inset=-.02,legend=c('High Bacteria','Low Bacteria'),pch=22,pt.cex=2.3,horiz=T,bty='n',pt.bg=c('black','gray80'))
...
mantel.test(gendist[,2:9],as.matrix(physiodist))
